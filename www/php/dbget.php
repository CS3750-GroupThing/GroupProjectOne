<?php
	...
	*/
	...
	if (isset($_GET['username']) && isset($_GET['password']) && isset($_GET['action']))
	{
		...
		if ($post_Action == "ADD")
		{
			if ((isset($_GET['hash'])) && (isset($_GET['data'])) && (isset($_GET['completed'])))
			{
				$post_Hash = $_GET['hash'];
				$post_Data = $_GET['data'];
				$post_Completed = $_GET['completed'];
			} else {
		...
		if (($row['username'] == $post_Username) && ($row['password'] == $post_Password))
		{
			if ($post_Action == "GET") 
			{
				...
				$sql = "DELETE FROM `todo_Items` WHERE owner=".$owner_ID." AND hash='".$post_Hash."'";
				echo $sql."<br /><br />";
				$result = mysqli_query($connection, $sql) or die("Error: " . mysqli_error($connection));
			} else if ($post_Action == "ADD") {
				$sql = "INSERT INTO `todo_Items` (`owner`, `completed`, `hash`, `description`) VALUES ($owner_ID, $post_Completed, '$post_Hash','$post_Data');";
				$result = mysqli_query($connection, $sql) or die("Error: " . mysqli_error($connection));
			} else {
				die("Failure: Invalid Action");
			}
